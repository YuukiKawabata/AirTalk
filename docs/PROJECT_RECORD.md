...
最終更新: 2026-06-29 / 状態: **AirTalk Plus 1.2(7) 再提出準備済み**
...
| 現在のビルド | **1.0(4)** が公開中 / **1.2(7)** をアップロード済み、App Store version 1.2 に紐づけ済み |
...
| 2026-06-29 時点のASC状態 | 商品・全地域価格・日本語ローカライズ・審査用スクリーンショット登録済み。サブスクリプション2商品は `READY_TO_SUBMIT`。アプリ本体 `1.2(7)` は `PREPARE_FOR_SUBMISSION` |
...
```
現在: 1.0(4) 公開中 / AirTalk Plus 1.2(7) 再提出準備済み
   ↓
App Store Connect Web UI で 1.2 の提出にサブスクリプション2商品を追加
   ↓
再提出 → 承認待ち
```
...
- 初回サブスクリプション2商品は `READY_TO_SUBMIT`。Apple 公開 API では `reviewSubmissionItems` にサブスクリプションを追加できないため、App Store Connect Web UI で 1.2 提出に追加する
